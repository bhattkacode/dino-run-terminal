...
class Obstacle:
    def __init__(self):
        ...
        self.oy = b-choices(range(self.oy2, 10),
                            weights=(90, 15, 25, 10, 5, 20, 25), k=1)[0]
        self.lst = []
    ...
    def render(self):
        ...
        for i in range(self.oy2):
            self.lst.append(
                range(self.ox+i*d+self.oy*d, self.ox2+i*d+self.oy*d))
        for i in self.lst:
            gassign(i)
...
if startslow:
    pmag = 0.04     # Player y speed(jump speed)
    gmag = 0.013    # Player x Speed(ground move speed)
...
while True:
    ...
    if collision():
        ...
        input(" "*(l//2-23)+"Press Enter to Restart, Ctrl+c to exit")
        u = ul
        obstacles = []
        out = list((" "*l+"\n")*b)
        assign(range(pi+1+u*d, pi+3+u*d))
